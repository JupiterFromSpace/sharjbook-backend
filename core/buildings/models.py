...
class Building(models.Model):
    ...
    image = models.ImageField(upload_to = building_image_upload_path, null=True , blank=True , verbose_name='عکس از ساختمان ')
    building_type = models.CharField(max_length=20, choices=BuildingTypes.choices, verbose_name='نوع ساختمان')
    use_type = models.CharField(max_length=20, choices=BuildingUseTypes.choices, verbose_name='کاربری ساختمان')
    
    manager = models.ForeignKey(user, on_delete=models.CASCADE, related_name='managed_buildings',
    limit_choices_to = {'role':'MANAGER'}, verbose_name='مدیر ساختمان'
    )
